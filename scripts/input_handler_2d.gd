...
func _get_cell_under_mouse(mouse_pos: Vector2) -> Node:
	...
	for coord in grid_manager.cells:
		...
		if cell and cell is Area2D:
			var shape = cell.get_node_or_null("CollisionShape2D")
			if shape and shape.shape:
				var global_pos = cell.global_position
				var shape_size = shape.shape.size if shape.shape is RectangleShape2D else Vector2(40, 45)
