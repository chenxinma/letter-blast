	...
	for row in range(grid_height):
		for col in range(grid_width):
			...
			if cells[coord] == null:
				...
				var pos_x = (col - grid_width / 2.0 + 0.5) * CELL_SIZE_3D * cell_spacing
...
func do_place_word_by_path(word: String, path: Array[Vector2]) -> void:
	for i in range(path.size()):
		...
		if existing_cell == null:
			...
			var pos_x = (coord.x - grid_width / 2.0 + 0.5) * CELL_SIZE_3D * cell_spacing
			var pos_y = (grid_height / 2.0 - coord.y - 0.5) * CELL_SIZE_3D * cell_spacing
			cell_instance.position = Vector3(pos_x, pos_y, 0)
